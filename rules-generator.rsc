:global zones {"LAN";"WAN";"DMZ";"WIFI";"VPN"}
...
:foreach zone in=$zones do={
    ...
    :foreach zone in=$zones do={
        ...
        :if ($targetzone=$zone) do={
            ...
            add chain=$Intrazone
            add action=passthrough chain=$ZonetoFirewall comment="SZ:$zone to SZ:Firewall - Accept"
            add action=accept chain=$ZonetoFirewall comment="$zone-to-Firewall - Accept"
            add action=passthrough chain=$FirewalltoZone comment="SZ:Firewall to SZ:$zone - Accept"
            ...
            add action=passthrough chain=$Targetzonetozone comment="SZ:$targetzone to SZ:$zone";
